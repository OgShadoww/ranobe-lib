...
const Main = () => {
    ...
    return (
        <div className='lg:px-[10%] md:px-[5%] sm:px-0 px-0 mb-[20px] lg:mt-[15px] md:mt-[15px] sm:mt-0 mt-0 w-full flex lg:justify-around justify-center gap-[20px]'>
            <div className='flex flex-col gap-[15px] page-wrraper'>
                <div className='flex flex-col items-start gap-[10px] main-wrapper'>
                    <h3 className='font-semibold text-[22px]'>Випадкове ранобе</h3>
                    <MainRandomList/>
                </div>
                <div className='flex flex-col lg:flex-row md:flex-col sm:flex-col gap-[20px]'>
                    <MainForumBlock title='Остальні новини' type='news'/>
                    <MainForumBlock title='Остальні теми форума' type='all'/>
                </div>
                <BigListRanobe/>
            </div>
            <div className='lg:flex lg:flex-col w-[335px] hidden'>
                <MainRightWrapper title='Жанри'>
                    <MainGenresList minHeight={open}/>
                    <div onClick={() => setOpen(!open)} className='tags-more'>
                        Розвернути
                        <AiFillCaretDown/>
                    </div>
                </MainRightWrapper>
                <MainRightWrapper title='Найпопулярніші ранобе'>
                    <PopularRanobeList/>
                </MainRightWrapper>
            </div>
        </div>
